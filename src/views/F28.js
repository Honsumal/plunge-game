...
export default function F28 ({toMenu, toNext, aLv, setALv, bLv, setBLv, cLv, setCLv}) {
    ...
    const field = 'Superheated'

    const {
        turn,
        inSeq,
        announcerMessage
    } = useF28BattleSequence(sequence, active, a, b, c, e, setRotating, setActive, field)
    ...
    return (
        <div>
            <BattleField announcerMessage={announcerMessage} activechar={active} echar={e} setRotating={setRotating} setSequence={setSequence} setActive={setActive} handleCloseStart={handleCloseStart} turn={turn} rotating={rotating} toMenu={toMenu} toNext={toNext} open={open} openStart={openStart} a={a} b={b} c={c} floor = {28} field={field}/>
        </div>
    )
}