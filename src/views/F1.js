...
export default function F1 ({toMenu, toNext, aLv, setALv, bLv, setBLv, cLv, setCLv}) {
    ...
    const {
        ...
        inSeq,
        announcerMessage
    } = useF1BattleSequence(sequence, active, a, b, c, e, setRotating, setActive, field)
    ...
    return (
        <div>
            <BattleField announcerMessage={announcerMessage} activechar={active} echar={e} setRotating={setRotating} setSequence={setSequence} setActive={setActive} handleCloseStart={handleCloseStart} turn={turn} rotating={rotating} toMenu={toMenu} toNext={toNext} open={open} openStart={openStart} a={a} b={b} c={c} floor={1} field = {field}/>
        </div>
    )
}